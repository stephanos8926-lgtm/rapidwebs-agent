...
- Project-Specific Memories Are HIGH VALUE: RapidWebs-agent project memories scored 83% admit rate (avg 7.88 composite score) using A-MAC algorithm. These represent hard-won integration knowledge (TUI, duration tracking, MCP patterns) and should NOT be removed lightly. Only remove: conversational filler, API keys (security), resolved/superseded info, raw tool output. Keep: architecture decisions, error prevention patterns, tool configurations.
- Memory Optimization Tools Available: (1) extract_memories.py - extracts memories from QWEN.md to JSON, (2) memory_scorer.py - scores using A-MAC algorithm, (3) memory_optimizer.py - complete workflow. Usage: python tools/memory_optimizer.py --project --report-only. Pre-commit hook at hooks/pre-commit.bat checks memories before commits (warning-only). Run monthly to prevent memory bloat.
